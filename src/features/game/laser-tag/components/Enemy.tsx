...
import { useGameStore, type EnemyData } from '../store/index';
...
export function Enemy({ data }: { data: EnemyData }) {
  ...
  useEffect(() => {
    ...
    const rngLocal = rng(parseInt(data.id.split('-')[1]) || 123);
    patrolTarget.current.set(
      data.position[0] + (rngLocal() - 0.5) * 10,
      data.position[1],
      data.position[2] + (rngLocal() - 0.5) * 10
    );
